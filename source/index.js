...
async function main() {
  ...
  await options.askWipeSystemGradleCache();
  await options.askNodeModules();
  await options.askResetMetroCache();
  ...
  if (options.getWipeNodeModules()) {
    ...
    await executeTask(tasks.pnpmStorePrune);
    await executeTask(tasks.bunCacheClean);
    await executeTask(tasks.npmInstall);
    await executeTask(tasks.yarnInstall);
    await executeTask(tasks.pnpmInstall);
    await executeTask(tasks.bunInstall);
  }

  if (options.getWipeSystemGradleCache()) {
    await executeTask(tasks.wipeSystemGradleCache);
  }
  ...
  if (options.getResetMetroCache()) {
    await executeTask(tasks.resetMetroCache);
  }
}
